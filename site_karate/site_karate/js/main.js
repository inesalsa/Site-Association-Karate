...
document.addEventListener("DOMContentLoaded", () => {
    ...
    fetch('pages/get_user.php')
        ...
        .then(data => {
            console.log("Réponse du serveur :", data); // Tu verras ça dans ta console !

            if (data.loggedin) {
                const navLinks = document.querySelectorAll('nav a');
                navLinks.forEach(link => {
                    // On vérifie si le lien pointe vers la page de login
                    if (link.href.includes('login.html')) {
                        link.innerHTML = "🥋 Mon Profil";
                        link.href = "profile.php"; // profile.php est à la racine selon ton image
                        link.style.color = "#bc002d"; // Rouge karaté
